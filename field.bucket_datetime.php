 ...
 */
class Field_bucket_datetime
{	
	public $field_type_slug			= 'bucket_datetime';
	
	public $db_col_type				= 'datetime';

	public $custom_parameters		= array('use_time', 'storage', 'input_type');
	...
	public function validate($value, $mode, $field)
	{
		...
		// a required field.
		$field_data = $this->CI->form_validation->field_data($field->field_slug.'_datepicker');
		...
		if (is_array($restrict = $this->parse_restrict($field->field_data)))
		{
			...
			if ( ! $restrict['start_stamp'] and $restrict['end_stamp'])
			{
				...
				if ($value > $pieces[1])
				{
					return lang('streams.flexi_date_out_or_range');
				}
			}
	...
	public function form_output($data, $entry_id, $field)
	{
		...
		// Create a unique instance this will 
		// help keep things unique for JS
		$instance = rand(0, 1000).rand(0, 1000);


		// -------------------------------------
		// Build the flexi part with buckets
		// -------------------------------------
		// This will help save time when exact
		// values just don't matter.
		// -------------------------------------
		...
		if ( empty($data['value']) )
		{
			$bucket_value = strtotime('today 9:00 am');
			$is_a_bucket = true;
		}
		else
		{
			...
			// Is it a bucket one?
			$tmp = strtotime($data['value']);

			if ( isset($buckets[$tmp]) )
			{
				$bucket_value = $tmp;
				$is_a_bucket = true;
			}
		...
		$js = '
			...
			<script type="text/javascript">
				...
				$(".bucket_selector_'.$instance.'").change(function(){

					if ( $(this).val() == "" )
					{
						$(".specific_details_'.$instance.'").show();
					}
					else
					{
						$(".specific_details_'.$instance.'").hide();
					}
		...
		$html = form_dropdown(
			$data['form_slug'].'_bucket',
			$buckets,
			$bucket_value,
			'class="skip bucket_selector bucket_selector_'.$instance.'"');
		...
		$date = $this->get_date(trim($data['value']), $data['form_slug'].'_datepicker', $data['custom']['use_time']);
		...
		if ($input_type == 'datepicker')
		{
			...
			$options['name'] 	= $data['form_slug'].'_datepicker';
		...
		$html .= '<div class="specific_details specific_details_'.$instance.'" style="display: '.($is_a_bucket == true ? 'none' : null).';">';
		$html .= $date_input;
		$html .= '</div>';
	...
	public function pre_save($input, $field)
	{
		...
		if ($input_type == 'datepicker')
		{
			...
			$date = $this->CI->input->post($field->field_slug.'_datepicker');
		}
		else
		{
			// Get from post data
		...
		// -------------------------------------
		// Wait.. are we using a bucket?
		// -------------------------------------

		if ( isset($_POST[$field->field_slug.'_bucket']) AND $_POST[$field->field_slug.'_bucket'] != null )
		{
			$date = date('Y-m-d H:i:s', $_POST[$field->field_slug.'_bucket']);
		}
	...
	/**
	 * Make bucket time / values
	 *
	 * @access	private
	 * @return	array
	 */	
	private function make_buckets()
	{
		return array(
			// Make the times
			'today' 	=> strtotime('today 9:00 am'),
			'tomorrow' 	=> strtotime('tomorrow 9:00 am'),
			'this_week' => strtotime('this friday 9:00 am'),
			'next_week' => strtotime('next friday 9:00 am'),
			'later' 	=> strtotime('+2 weeks friday 9:00 am'),
			);
	}
	...
	/**
	 * Get and return an array of bucket option / values
	 *
	 * @access	private
	 * @return	array
	 */	
	private function get_buckets()
	{
		...
		$buckets = $this->make_buckets();

		return array(
			$buckets['today'] 		=> lang('streams.bucket_datetime.option_today'),
			$buckets['tomorrow'] 	=> lang('streams.bucket_datetime.option_tomorrow'),
			$buckets['this_week'] 	=> lang('streams.bucket_datetime.option_this_week'),
			$buckets['next_week'] 	=> lang('streams.bucket_datetime.option_next_week'),
			$buckets['later'] 		=> lang('streams.bucket_datetime.option_later'),
			null 					=> lang('streams.bucket_datetime.option_specific'),
			);
	}
}